...
# Rota para processar o formulário e gerar os DataFrames - BIBLIOTECA PANDAS 
@app.route('/formulario_relatorio2', methods=['POST'])
def formulario_relatorio2():
    ...
    try:
        ...
        df_funcionarios_filtrado = df_funcionarios[df_funcionarios['obra_id'] == obra_id]
        df_obras_filtrado = df_obras[df_obras['obra_id'] == obra_id]
        df_presenca_obras_filtrado = df_presenca_obras[(df_presenca_obras['obra_id'] == obra_id) & (df_presenca_obras['data'] >= periodo_inicial) & (df_presenca_obras['data'] <= periodo_final)]
  
  
        # Unir DataFrames
        df_relatorio_detalhado = pd.merge(df_presenca_obras_filtrado, df_funcionarios_filtrado, on='Id_funcionario', how='inner')
        df_relatorio_detalhado = pd.merge(df_relatorio_detalhado, df_obras_filtrado, on='id_Obra', how='inner')
